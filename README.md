...
A layered TypeScript client toolkit for the [Agent Client Protocol (ACP)](https://agentclientprotocol.com). acpjs plays the **Client role** in ACP: it spawns and connects to any ACP-compliant agent process through the official SDK, projects ACP session traffic into a serializable, replayable, typed event stream and session state, and delivers that state to native Node, React, and Electron consumers that barely have to touch the protocol.
...
- **Typed, closed surface.** Every stable protocol capability has a typed entry point. There is no escape hatch for sending raw ACP methods outside the public surface — when the protocol grows, the toolkit grows with it.
- **Serializable host contract.** The acpjs host/client boundary is a HostClientTransport contract (request/response + event push + reverse requests). Payloads are structured-clone safe, so the same client works in-process or across a process boundary, shipping only small normalized event increments.
...
```
                @acpjs/protocol
   (types + pure reducer + host transport contract; environment-neutral, zero runtime deps)
                  ▲          ▲          ▲
        ┌─────────┘          │          └──────────┐
...
| Package                | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Docs                                         |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `@acpjs/protocol`      | The normalized `AcpjsEvent` projection model, the `SessionState` model, the pure `reduce` reducer, and the acpjs host transport contract types. Environment-neutral: types and pure functions only.                                                                                                                                                                                                                                                   | [README](./packages/protocol/README.md)      |
...
acpjs deliberately stays a typed entry point to **stable** protocol capabilities — no raw ACP-method escape hatch, no business policy layer, and no redefinition of the ACP wire protocol. The following are intentional non-goals, owned by the integrator, not gaps:

- **Authentication.** acpjs surfaces the ACP `authenticate` and `logout` RPCs as typed mechanism (`agent.authenticate(methodId)` / `agent.logout()`) — it sends the call, nothing more. It stores no credentials, tracks no login state, and never picks a method on the integrator's behalf. Any auth requirement — including MCP auth — still propagates upward as the agent's authentication error; the integrator chooses which advertised `authMethod` to use (surfaced verbatim on the agent snapshot as `authMethods`), decides when to authenticate, and retries. Sending the RPC is mechanism; choosing the method and timing is the integrator's decision.
...
`createAcpHost` starts a host, `createHostEndpoint` wraps it as a HostClientTransport endpoint, and `createInProcessTransport` connects it to `createAcpClient` via direct function calls (no serialization, no transport boundary). From there you `spawn`, `create` a session, `subscribe`, and `prompt`.
...
- `/renderer` — `electronTransport()` produces a HostClientTransport-contract transport to pass to `createAcpClient`.
