...
- **host method ids**: sourced from `@acpjs/protocol`'s `ACPJS_HOST_METHODS` (`agents/spawn|list|dispose`, `sessions/create|load|list|resume|delete|prompt|cancel|close|setMode|setConfigOption|getAll|restore`), the same constant table consumed by core's `createHostEndpoint` (pinned by protocol and end-to-end tests). These are acpjs host-client adapter ids, not ACP agent method names.
